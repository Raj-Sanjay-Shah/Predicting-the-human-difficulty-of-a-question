...
flags.DEFINE_integer('early_stopping', 2,
...
f = open('GCN/data/corpus/' + dataset + '_vocab.txt', 'r')
words = f.readlines()
f.close()
...
word_embeddings_str = '\n'.join(word_vectors)
f = open('GCN/data/' + dataset + '_word_vectors.txt', 'w')
...
gcn_vectors = np.concatenate((train_doc_embeddings,test_doc_embeddings))
with open(str(parent_path) + '/Data/doc_vectors.txt', 'wb') as fh:
   pickle.dump(gcn_vectors, fh)

doc_embeddings_str = '\n'.join(doc_vectors)
f = open('GCN/data/' + dataset + '_doc_vectors.txt', 'w')
